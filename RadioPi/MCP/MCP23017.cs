...
namespace RadioPi.MCP
{
    public sealed class MCP23017
    {
        ...
        public string I2C_CONTROLLER_NAME { get { return "I2C1"; } } //specific to RPI2
        ...
        {
            Addresses.SetDeviceIndex(deviceIndex);
        }

        public Windows.Foundation.IAsyncOperation<bool> Init()
        {
        ...
        private async Task<bool> init()
        {
            ...
            var i2cSettings = new I2cConnectionSettings(Addresses.PORT_EXPANDER_I2C_ADDRESS);
            ...
            try
            {
                ...
                i2CWriteBuffer = new byte[] { Addresses.PORT_EXPANDER_GPPUA_ADDRESS, 0x00 };
                i2cPortExpander.Write(i2CWriteBuffer);

                i2CWriteBuffer = new byte[] { Addresses.PORT_EXPANDER_GPPUB_ADDRESS, 0x00 };
                i2cPortExpander.Write(i2CWriteBuffer);

                //Set all to outputs
                i2CWriteBuffer = new byte[] { Addresses.PORT_EXPANDER_IODIRA_REGISTER_ADDRESS, 0x00 };
                i2cPortExpander.Write(i2CWriteBuffer);

                i2CWriteBuffer = new byte[] { Addresses.PORT_EXPANDER_IODIRB_REGISTER_ADDRESS, 0x00 };
                i2cPortExpander.Write(i2CWriteBuffer);

                //reset input polarity
                i2CWriteBuffer = new byte[] { Addresses.PORT_EXPANDER_IPOLA_REGISTER_ADDRESS, 0x00 };
                i2cPortExpander.Write(i2CWriteBuffer);

                i2CWriteBuffer = new byte[] { Addresses.PORT_EXPANDER_IPOLB_REGISTER_ADDRESS, 0x00 };
                i2cPortExpander.Write(i2CWriteBuffer);

                //configure device for open-drain output on the interrupt pin
                i2CWriteBuffer = new byte[] { Addresses.PORT_EXPANDER_IOCON_ADDRESS, 0x02 };
                i2cPortExpander.Write(i2CWriteBuffer);
        ...
        private void GetRegisterValues()
        {
            ...
            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_IODIRA_REGISTER_ADDRESS }, i2CReadBuffer);
            iodirARegister = i2CReadBuffer[0];

            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_GPIOA_REGISTER_ADDRESS }, i2CReadBuffer);
            gpioARegister = i2CReadBuffer[0];

            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_OLATA_REGISTER_ADDRESS }, i2CReadBuffer);
            olatARegister = i2CReadBuffer[0];

            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_IODIRB_REGISTER_ADDRESS }, i2CReadBuffer);
            iodirBRegister = i2CReadBuffer[0];

            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_GPIOB_REGISTER_ADDRESS }, i2CReadBuffer);
            gpioBRegister = i2CReadBuffer[0];

            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_OLATB_REGISTER_ADDRESS }, i2CReadBuffer);
            olatBRegister = i2CReadBuffer[0];

            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_GPINTA_ADDRESS }, i2CReadBuffer);
            gpintARegister = i2CReadBuffer[0];

            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_GPINTB_ADDRESS }, i2CReadBuffer);
            gpintBRegister = i2CReadBuffer[0];

            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_INTCONA_ADDRESS }, i2CReadBuffer);
            intconARegister = i2CReadBuffer[0];

            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_INTCONB_ADDRESS }, i2CReadBuffer);
            intconBRegister = i2CReadBuffer[0];

            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_IOCON_ADDRESS }, i2CReadBuffer);
        ...
        public void SetDriveMode(Pin pin, PinMode pinMode)
        {
            ...
            {
                PORT_EXPANDER_OLAT_REGISTER_ADDRESS = Addresses.PORT_EXPANDER_OLATA_REGISTER_ADDRESS;
                PORT_EXPANDER_IODIR_REGISTER_ADDRESS = Addresses.PORT_EXPANDER_IODIRA_REGISTER_ADDRESS;
            ...
            {
                PORT_EXPANDER_OLAT_REGISTER_ADDRESS = Addresses.PORT_EXPANDER_OLATB_REGISTER_ADDRESS;
                PORT_EXPANDER_IODIR_REGISTER_ADDRESS = Addresses.PORT_EXPANDER_IODIRB_REGISTER_ADDRESS;
        ...
        public IEnumerable<Pin> GetChangedPins()
        {
            ...
            i2CReadBuffer = new byte[1];
            //Check A PINs
            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_INTFA_ADDRESS }, i2CReadBuffer);
            ...
            //Check B PINS
            i2cPortExpander.WriteRead(new byte[] { Addresses.PORT_EXPANDER_INTFB_ADDRESS }, i2CReadBuffer);
        ...
        public void SetInterruptOnChange(Pin pin, bool setInterruptOn)
        {
            ...
            if (pinName.Substring(2, 1) == "A")
            {
                PORT_EXPANDER_INTERRUPT_ON_CHANGE_ADDRESS = Addresses.PORT_EXPANDER_GPINTA_ADDRESS;
                PORT_EXPANDER_INTCON_ADDRESS = Addresses.PORT_EXPANDER_INTCONA_ADDRESS;
            ...
            if (pinName.Substring(2, 1) == "B")
            {
                PORT_EXPANDER_INTERRUPT_ON_CHANGE_ADDRESS = Addresses.PORT_EXPANDER_GPINTB_ADDRESS;
                PORT_EXPANDER_INTCON_ADDRESS = Addresses.PORT_EXPANDER_INTCONB_ADDRESS;
        ...
        {
            i2cPortExpander.Write(new byte[] { reg, value });
        }
        ...
        public bool Read(Pin pin)
        {
            ...
            if (pinName.Substring(2, 1) == "A")
            {
                PORT_EXPANDER_GPIO_REGISTER_ADDRESS = Addresses.PORT_EXPANDER_GPIOA_REGISTER_ADDRESS;
            }

            if (pinName.Substring(2, 1) == "B")
            {
                PORT_EXPANDER_GPIO_REGISTER_ADDRESS = Addresses.PORT_EXPANDER_GPIOB_REGISTER_ADDRESS;
            }
